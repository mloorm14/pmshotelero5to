    ...
    <div>
      {occupiedRooms.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-zinc-700 bg-zinc-900 p-12 text-center shadow-sm">
          ...
          <p className="mt-4 text-lg font-medium text-zinc-200">No hay habitaciones ocupadas</p>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          {occupiedRooms.map((room) => {
            ...
            return (
              <article
                key={room.id}
                className="rounded-2xl border border-red-900/40 bg-zinc-900 p-6 shadow-sm"
              >
                ...
                {room.guest && (
                  <dl className="mb-5 space-y-2 text-sm">
                    <div className="flex justify-between">
                      <dt className="text-zinc-500">Huésped</dt>
                      <dd className="font-medium text-zinc-200">{room.guest.fullName}</dd>
                    </div>
                    <div className="flex justify-between">
                      <dt className="text-zinc-500">Documento</dt>
                      <dd className="text-zinc-300">{room.guest.documentId}</dd>
                    </div>
                    <div className="flex justify-between">
                      <dt className="text-zinc-500">Teléfono</dt>
                      <dd className="text-zinc-300">{room.guest.phone}</dd>
                    </div>
                    {room.billing?.checkInDate && (
                      <div className="flex justify-between">
                        <dt className="text-zinc-500">Estadía</dt>
                    ...
                    {room.billing && (
                      <div className="flex justify-between border-t border-zinc-800 pt-2">
                        <dt className="text-zinc-500">Total cobrado</dt>
                        <dd className="text-lg font-bold text-red-300">
                ...
                <button
                  ...
                  className="w-full rounded-xl bg-amber-700 px-4 py-3 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-amber-600 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 focus:ring-offset-zinc-900"
                >
                  Procesar Check-out
                </button>
              </article>
